...
def build_val_transform(val_input_size, val_label_size):
    ...
    transforms = []
    transforms.append(T.ValResize(val_input_size, val_label_size))
    transforms.append(T.ToTensor())
    transforms.append(T.Normalize(mean, std))
    return T.Compose(transforms)


def build_train_transform2(
    ...
    resize_mode = "uniform"
    transforms = []
    transforms.append(T.RandomResize(train_min_size, train_max_size, resize_mode))
    if isinstance(train_crop_size, int):
        crop_h, crop_w = train_crop_size, train_crop_size
    else:
        crop_h, crop_w = train_crop_size
    transforms.append(T.RandomCrop2(crop_h, crop_w, edge_aware=edge_aware_crop))
    ...
    if aug_mode == "baseline":
        ...
        transforms.append(
            T.RandAugment(
                2, 0.2, "full", prob=1.0, fill=fill, ignore_value=ignore_value
            )
        )
    elif aug_mode == "randaug_reduced":
        transforms.append(
            T.RandAugment(
                2, 0.2, "reduced", prob=1.0, fill=fill, ignore_value=ignore_value
            )
        )
        ...
        transforms.append(
            T.RandomRotation(
                (-10, 10), mean=fill, ignore_value=ignore_value, prob=1.0, expand=False
            )
        )
    elif aug_mode == "noise":
        ...
        transforms.append(
            T.RandAugment(
                2, 0.2, "reduced2", prob=1.0, fill=fill, ignore_value=ignore_value
            )
        )
...
def build_train_transform(
    ...
    if aug_mode == "baseline":
        ...
        transforms.append(
            T.RandAugment(
                2, 0.3, "reduced2", prob=1.0, fill=fill, ignore_value=ignore_value
            )
        )
    elif aug_mode == "randaug_reduced3":
        transforms.append(
            T.RandAugment(
                2, 0.3, "reduced", prob=1.0, fill=fill, ignore_value=ignore_value
            )
        )
    elif aug_mode == "colour_jitter":
        transforms.append(T.ColorJitter(0.3, 0.3, 0.3, 0, prob=1))
...
def get_cityscapes(
    ...
    train_transform = build_train_transform2(
        train_min_size, train_max_size, train_crop_size, aug_mode, ignore_value
    )
    ...
    train = Cityscapes(
        ...
        class_uniform_pct=class_uniform_pct,
    )
    val = Cityscapes(
        root,
        split=val_split,
