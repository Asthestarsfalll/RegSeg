    ...
    for city in sorted(os.listdir(images_dir)):
        ...
        for file_name in sorted(os.listdir(img_dir)):
            ...
            target_name = "{}_{}".format(
                file_name.split("_leftImg8bit")[0], target_suffix
            )
    ...
    def __init__(
        ...
        if "train" in self.split:
            self.all_imgs.extend(
                find_cityscapes_filenames(
                    self.images_dir, self.targets_dir, "train", target_suffix
                )
            )
        if "val" in self.split:
            self.all_imgs.extend(
                find_cityscapes_filenames(
                    self.images_dir, self.targets_dir, "val", target_suffix
                )
            )
        if "test" in self.split:
            self.all_imgs.extend(
                find_cityscapes_filenames(
                    self.images_dir, self.targets_dir, "test", target_suffix
                )
            )
        if self.class_uniform_pct > 0 and self.split in ["train", "trainval"]:
            ...
            self.centroids = uniform.build_centroids(
                self.all_imgs, self.num_classes, id2trainid=False, json_fn=json_fn
            )
        else:
            self.centroids = []
        self.build_epoch()
    ...
    def build_epoch(self):
        ...
        if len(self.centroids) != 0:
            self.imgs = uniform.build_epoch(
                self.all_imgs, self.centroids, self.num_classes, self.class_uniform_pct
            )
        else:
