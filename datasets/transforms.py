...
from datasets.augment import rand_augment_both
...
class ValResize(object):
    ...
    def __call__(self, image, target):
        ...
        target = F.resize(
            target, self.label_size, interpolation=F.InterpolationMode.NEAREST
        )
        return image, target
...
class RandomResize(object):
    ...
    def __call__(self, image, target):
        if self.sampling_mode == "uniform":
            ...
            size = int(
                2 ** random.uniform(math.log2(self.min_size), math.log2(self.max_size))
            )
        else:
            raise NotImplementedError()
        if isinstance(target, tuple) or isinstance(target, list):
...
class RandomPad(object):
    ...
    def __call__(self, image, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            image = F.pad(
                image, (pad_left, pad_top, pad_right, pad_bottom), fill=self.pad_value
            )
            label = F.pad(
                label,
...
class RandomCrop(object):
    ...
    def __call__(self, image, label):
        ...
        if centroid is not None:
            crop_params = get_centroid_crop_params(
                image, (self.crop_h, self.crop_w), centroid
            )
